...
public class TripAnalysis extends TripAnalysisResult {
    ...
    public void analyze() {
        ...
        setEntryDeparture(new DateTime(getEntryRT().get(getEntryRT().size() - 1).getCurrentTime()));      //obviously the next minute!! though might result in a timespan of 0 min due to sample data)!
