...
public class AnalysisService {
    ...
    public void jmsAnalyseTarget(AnalysisRequestDTO analysisRequestDTO) {
        ...
        List<RealtimeData> startStationRealtime = commandFactory.getRealtimeDataByStationAndTWCommand(analysisRequestDTO.getLine(), direction.getName(), entryStation.getNumber(), analysisRequestDTO.getFrom(), analysisRequestDTO.getTo()).toList();

        // start analyzing

        List<Date> arrivalTimesEntry = startStationRealtime.stream().map(x -> x.getPlannedArrival()).distinct().collect(Collectors.toList());
        List<Map<Integer, List<RealtimeData>>> differentTrainsEntry = splitDifferentTrains(startStationRealtime);
        startStationRealtime = null;    // for the GC

        

     //   followTrains(differentTrainsEntry, analysisRequestDTO.getLine(), direction.getName(), analysisRequestDTO.getFrom(), analysisRequestDTO.getTo(), stations, entryStation, exitStation);

        //List<Map<Integer, List<RealtimeData>>> trainsExit = followTrains(analysisRequestDTO.getLine(), direction.getName(), analysisRequestDTO.getFrom(), analysisRequestDTO.getTo(), differentTrainsEntry, stations);


        ReportDTO reportDTO = new ReportDTO();
        reportDTO.setLines(Arrays.asList(new Line(analysisRequestDTO.getLine())));
        reportDTO.setTime(new ReportTimeSpan(analysisRequestDTO.getFrom(), analysisRequestDTO.getTo()));
        reportDTO.setStations(stations);
